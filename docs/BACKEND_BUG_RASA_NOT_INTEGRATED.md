...
1. ✅ **Rasa Integration Working** - Code đã có Rasa webhook call
2. ✅ **Added Logging** - Console logs để debug Rasa connection
3. ✅ **Added Timeout** - 10s timeout cho Rasa requests
4. ✅ **Better Error Handling** - Detailed error messages
5. ✅ **Default URL** - Fallback to `http://localhost:5005` nếu env không set

### Root Cause:

Code Rasa integration đã có từ trước, nhưng:
- ❌ Không có logging → Khó debug
- ❌ Error bị nuốt im → Frontend tưởng hardcoded
- ❌ Có thể RASA_SERVER_URL chưa được set trong `.env`

### Testing:

**Verify Rasa is called:**
```bash
# Terminal 1: Start Rasa
cd kltn_chatbot
rasa run --enable-api --cors "*"

# Terminal 2: Send message via backend
...
Nếu không set, sẽ dùng default `http://localhost:5005`
