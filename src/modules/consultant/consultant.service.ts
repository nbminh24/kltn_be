...
import { ValidateMixDto } from './dto/validate-mix.dto';
...
export class ConsultantService {
    ...
    async getStylingAdvice(dto: StylingAdviceDto) {
        ...
        if (style) {
            queryBuilder.andWhere(
                "product.attributes->>'style' ILIKE :style",
                { style: `%${style}%` }
            );
        }
    ...
    private generateStylingTip(occasion: string, style: string): string {
        const tips = {
            wedding: 'Nên chọn vest hoặc áo sơ mi lịch sự, phối với quần tây và giày da. Tránh màu sắc quá chói.',
        ...
        return tips[occasion.toLowerCase()] || 'Hãy chọn trang phục phù hợp với dịp và phong cách của bạn.';
    }

    // Helper: Generate sizing advice
    private generateSizingAdvice(
        height: number,
        weight: number,
        size: string,
        fit: string
    ): string {
        return `Với chiều cao ${height}cm và cân nặng ${weight}kg, bạn nên mặc size ${size}. ${fit === 'Slim'
