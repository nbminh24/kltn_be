...
export class ChatController {
  ...
  @ApiOperation({
    ...
    description:
      'Tạo session mới cho guest (visitor_id) hoặc lấy session của customer đã login. JWT token tự động extract customer_id.',
  })
  @ApiResponse({ status: 201, description: 'Session được tạo hoặc lấy thành công' })
  createSession(@Body() dto: CreateSessionDto, @Headers('authorization') authHeader?: string) {
  ...
  sendMessage(@Body() dto: SendMessageDto, @Headers('authorization') authHeader?: string) {
  ...
  @ApiOperation({
    ...
    description:
      'Lấy danh sách chat sessions grouped by time. JWT token tự động extract customer_id.',
  })
  @ApiQuery({
    name: 'customer_id',
    required: false,
  ...
  getSessionsHistory(@Query() query: any, @Headers('authorization') authHeader?: string) {
  ...
  getActiveSession(@Query() query: any, @Headers('authorization') authHeader?: string) {
  ...
  @ApiOperation({
    ...
    description:
      'Upload ảnh để tìm các sản phẩm thời trang tương đồng. Trả về top 10 sản phẩm có độ tương đồng cao nhất.',
  })
  @ApiResponse({
    status: 200,
  ...
  async searchByImage(@UploadedFile() file: Express.Multer.File) {
    ...
    const products = await this.chatService.searchProductsByImage(file.buffer, file.originalname);
  ...
  @ApiOperation({
    ...
    description:
      'Endpoint cho Rasa custom action. Upload ảnh và nhận response dạng Rasa carousel để hiển thị cho user.',
  })
  @ApiResponse({
    status: 200,
