...
export class ImageSearchService {
  ...
  async searchByImage(imageBuffer: Buffer, filename: string): Promise<ImageSearchResponseDto> {
    try {
      ...
      this.logger.log(
        `✅ Image search completed in ${queryTime}ms, found ${response.data.results?.length || 0} results`,
      );
      ...
      throw new HttpException('Failed to process image search', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
