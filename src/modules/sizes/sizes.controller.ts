...
export class SizesController {
    ...
    @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Số items mỗi trang (default: 20)' })
    @ApiQuery({ name: 'search', required: false, type: String, description: 'Tìm kiếm theo tên size' })
    @ApiQuery({ name: 'sort', required: false, type: String, description: 'Sắp xếp (vd: sort_order:asc, name:asc)' })
    @ApiResponse({
        status: 200,
